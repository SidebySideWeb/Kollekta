...
function channelOrder(preferred, { hasEmail = false } = {}) {
  if (preferred && preferred !== 'auto' && AUTH_CHANNELS.includes(preferred)) {
    return [preferred, ...AUTH_CHANNELS.filter((c) => c !== preferred)];
  }
  // Prefer email first when available so codes don't silently land on Viber.
  if (hasEmail) {
    return ['email', ...AUTH_CHANNELS.filter((c) => c !== 'email')];
  }
  return AUTH_CHANNELS.filter((c) => c !== 'email');
}
...
async function tryChannel(customer, channel, kind, payload) {
  if (channel === 'email') {
    ...
    if (result.ok) {
      ...
      console.warn(`[messaging] email failed for customer ${customer.id}: ${result.error || 'unknown'}`);
    }
...
async function sendAuthMessage(customer, kind, { code }) {
  const attempts = [];
  const preferred = customer.preferred_channel || 'auto';
  const hasEmail = Boolean(customer.email);
  const channels = channelOrder(preferred, { hasEmail });

  for (const channel of channels) {
    // If admin explicitly chose email, do not silently fall back to Viber/SMS.
    if (preferred === 'email' && channel !== 'email') {
      break;
    }
    ...
    if (attempt.ok) {
      ...
      return {
        ...
        attempts,
        emailFailed: Boolean(emailAttempt && !emailAttempt.ok),
        emailError: emailAttempt && !emailAttempt.ok ? emailAttempt.error : null,
      };
    }
  }
