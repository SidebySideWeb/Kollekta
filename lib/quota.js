...
function getQuotaStatus() {
  ...
  const warnPercent = config.STORAGE_WARN_PERCENT;
  const criticalPercent = config.STORAGE_CRITICAL_PERCENT;
  ...
  let state = 'ok';
  if (percentUsed >= criticalPercent) state = 'full';
  else if (percentUsed >= warnPercent) state = 'warning';
...
function quotaExceededMessage(status) {
  ...
  return `Δεν υπάρχει αρκετός χώρος. Χρησιμοποιούνται ${used} από ${quota}. Αρχειοθέτησε παλιές συλλογές.`;
}
