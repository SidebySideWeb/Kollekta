...
/** Fixed dummy hash so missing-user checks still run timingSafeEqual work. */
const DUMMY_PASSWORD_HASH = hashSecret(
  'kollekta-dummy-password',
...
function authenticateAdmin(email, password) {
  ...
  // Always run a timingSafeEqual-backed verify (dummy hash if no user).
  const passwordOk = verifySecret(password, admin ? admin.password_hash : DUMMY_PASSWORD_HASH);
  if (!admin || !passwordOk) {
    return null;
  }
