...
function customerCanSeeCollection(customerId, collection) {
  // Downgrade safety: without FEATURE_TAGS, ignore visibility and show all.
  if (!config.FEATURE_TAGS) return true;
...
function getCollectionContext(customer, collectionId) {
  ...
  // Downgrade safety: keep order_items rows, but ignore them on Basic.
  const hasOrderData = Boolean(config.FEATURE_ORDER_FILTERING) && orderCount > 0;

  const orderRows = hasOrderData
    ? db
        .prepare(
...
function getAudienceStats(collectionId) {
  ...
  const selectedTags = config.FEATURE_TAGS ? parseTags(collection.visibility_tags) : [];
  const reachCount = getAudienceCustomers(collectionId).length;

  return {
    visibility: config.FEATURE_TAGS ? (collection.visibility || 'all') : 'all',
    selectedTags,
